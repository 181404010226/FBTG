...
def main():
    for model_name, cfg in model_cfgs.items():
        print(f"\n{model_name}:")
        print(f"  Embed dimensions: {cfg.embed_dim}")
        print(f"  Depths: {cfg.depths}")
        print(f"  Stem width: {cfg.stem_width}")
        print(f"  Head hidden size: {cfg.head_hidden_size}")
        
        # Calculate total number of layers
        total_layers = sum(cfg.depths)
        print(f"  Total layers: {total_layers}")
        
        # Calculate total parameters (approximate)
        total_params = 0
        for i in range(len(cfg.embed_dim)):
            if i == 0:
                total_params += cfg.stem_width * cfg.embed_dim[0] * 9  # Stem
            total_params += cfg.embed_dim[i] * cfg.embed_dim[i] * cfg.depths[i] * 4  # Rough estimate
        total_params += cfg.embed_dim[-1] * cfg.head_hidden_size  # Head
