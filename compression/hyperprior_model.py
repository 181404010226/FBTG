...
from compression import entropy_models, entropy_coding
from compression import compression_utils
...
class HyperpriorDensity(nn.Module):
    ...
    def cdf_logits(self, x, update_parameters=True):
        ...
        for k in range(len(self.filters)+1):
            ...
            # Move parameters to the same device as the input
            H_k, a_k, b_k = H_k.to(device), a_k.to(device), b_k.to(device)
