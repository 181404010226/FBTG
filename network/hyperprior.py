...
from helpers import maths, utils
from compression import hyperprior_model, prior_model
...
if __name__ == '__main__':
    ...
    def test_hyperprior_compression(input_shape):
        # Initialize Hyperprior
        hp = Hyperprior(bottleneck_capacity=220, entropy_code=True)
        
        # Create random input latents
        latents = torch.randn(input_shape)
        spatial_shape = input_shape[2:]
        
        print(f"Original latents shape: {latents.shape}")
        
        # Compress
        compression_output = hp.compress_forward(latents, spatial_shape)
        
        print(f"Compressed hyperlatents shape: {compression_output.hyperlatents_encoded.shape}")
        print(f"Compressed latents shape: {compression_output.latents_encoded.shape}")
        
        # Decompress
        decompressed_latents = hp.decompress_forward(compression_output, device='cpu')
        
        print(f"Decompressed latents shape: {decompressed_latents.shape}")
        
        # Calculate compression ratio
        original_size = np.prod(input_shape)
        compressed_size = len(compression_output.hyperlatents_encoded) + len(compression_output.latents_encoded)
        compression_ratio = original_size / compressed_size
        
        print(f"Compression ratio: {compression_ratio:.2f}")
        print(f"Bits per pixel: {compression_output.total_bpp:.4f}")
    ...
    for shape in test_shapes:
        ...
        test_hyperprior_compression(shape)